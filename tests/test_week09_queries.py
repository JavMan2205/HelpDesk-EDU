...
def create_ticket(service: TicketService, title: str, category: str, requester_id: int = 1):
...
def test_list_by_category_returns_only_matching_category():
    service = TicketService()
    hardware_ticket = create_ticket(service, "No imprime", "Hardware")
    software_ticket = create_ticket(service, "No ingresa", "Software")
    
    result = service.list_by_category("Hardware")
    assert result == [hardware_ticket]
    assert software_ticket not in result
